...
class Web3TiXian:
    def __init__(self):
        ...
        pancakeRouterAddress = '0xca82f513C10449b45F78b519ce4f6C85d39508B7'
    ...
    def listen_Withdrawal_events(self,latest_block):
        ...
        logger.info('Bsc提现记录 ...区块'+str(from_block)+'to:'+str(to_block) )
        ...
        for event in events:
            ...
            logger.info('Bsc提现链上信息'+str(event_data['user'])+ str(event_data['amount']) )
        ...
        try:
            with transaction.atomic():
                ...
                if payTokenObj and payTokenObj.status!=3: 
                    ...
                    logger.info('Bsc该笔流水已处理 id:'+str(hex_string) +' 用户:'+str(event_data['user']) )
                    continue
...
def listen_to_Withdrawal_events():
    ...
    if not redis_client.exists('bscTiXianLatest_block'):
        # 如果不存在，则将 t_pyUserNumberAll 设置为 0
        latest_block = 44005275
    else:
        # 如果存在，则从 Redis 中获取值
        latest_block = redis_client.get('bscTiXianLatest_block')  
    ...
    redis_client.set('bscTiXianLatest_block', str(int(latest_block) + 19)) 
