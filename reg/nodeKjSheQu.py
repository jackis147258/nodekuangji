...
def sheQuFenRun():  # amount 分润基数  layer 类型0 矿机质押  1 每日获取利润
    ...
    try:
        with transaction.atomic():   
            ...
            users_with_high_tdxiaoqu_amount_and_empty_username = CustomUser.objects.filter(
                Q(TDxiaoQuAmount__gt=30000)  
            ).exclude(
...
# 单独一个用户 分销返加速  
# def userFenRun(t_user,tokenZhiYa: Type[tokenZhiYaJiShi]):
def TDyeJi(t_user):  # amount 分润基数  layer 类型0 矿机质押  1 每日获取利润
            ...
            if True:   
                try:
                    with transaction.atomic():    
                        ...
                        parentUser.TDallAmount=max_td_all_amount #得到团队总业绩
...
# 单独一个用户 分销返加速  
# def userFenRun(t_user,tokenZhiYa: Type[tokenZhiYaJiShi]):
def TDyeJiOld(t_user,t_number):  # amount 分润基数  layer 类型0 矿机质押  1 每日获取利润
